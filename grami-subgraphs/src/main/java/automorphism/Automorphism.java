...
public class Automorphism <NodeType, EdgeType>
{
	...
	public Automorphism(HPListGraph<NodeType, EdgeType> graph) 
	{
		...
		for (int i = 0; i < graph.getNodeCount(); i++) 
		{
			...
			x.add((Integer)graph.getNodeLabel(i));
			
			myNode newNode= new myNode(i,x);
		...
		for (int i = 0; i < graph.getNodeCount(); i++) 
		{
			...
			HashMap<Integer,myNode> currentLabelNodes= nodesByLabel.get(currentNode.getLabel().get(0));
			if(currentLabelNodes==null)
			{
				currentLabelNodes= new HashMap<Integer, myNode>();
				nodesByLabel.put(currentNode.getLabel().get(0), currentLabelNodes);
			}
